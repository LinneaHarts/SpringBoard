...
) AS patron_name, f.name AS facility_name
...
AND m.surname != 'GUEST'
ORDER BY m.surname, facility_name
...
--Subqueries are often a smart way to begin thinking about queries but don't make much sense here where
--all tables have columns in the final select
SELECT f.name, CONCAT( m.firstname, ' ', m.surname ) AS membername, (
	CASE WHEN b.memid =0
	THEN f.guestcost * b.slots
	ELSE f.membercost * b.slots
